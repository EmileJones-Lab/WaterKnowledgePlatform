...
@Service
class FileNodeMilvusRepository(
    private val client: MilvusClientV2,
    milvusConfig: MilvusConfig,
    openAiConfig: OpenAiConfig
) : IFileNodeMilvusRepository {
    ...
    private val logger = LoggerFactory.getLogger(FileNodeMilvusRepository::class.java)
    ...
    override fun insert(collectionName: String, datum: FileNodeEmbeddingDatum): Boolean {
        checkCollectionExistOrCreate(collectionName)
        val jsonData = datum.toJsonObject(datum.vector.toFloatArray())
        val insertReq = InsertReq.builder().databaseName(databaseName).collectionName(collectionName).data(listOf(jsonData)).build()
        client.insert(insertReq)
        return true
    }

    override fun batchInsert(collectionName: String, data: List<FileNodeEmbeddingDatum>): Boolean {
        checkCollectionExistOrCreate(collectionName)
        val jsonObjectList = data.map { it.toJsonObject(it.vector.toFloatArray()) }.toMutableList()
        val insertReq = InsertReq.builder().databaseName(databaseName).collectionName(collectionName).data(jsonObjectList).build()
        client.insert(insertReq)
        return true
    }
    ...
    override fun batchDeleteByFileNodeIds(collectionName: String, fileNodeIds: List<String>): Boolean {
        ...
        val queryReq = QueryReq.builder().databaseName(databaseName).collectionName(collectionName).filter(filter).outputFields(listOf("fileNodeId", "vector", "isDelete")).build()
        val queryResults = client.query(queryReq).queryResults ?: emptyList()
        if (queryResults.isEmpty()) return true

        val tombstones = queryResults.mapNotNull { mapToEmbeddingDatum(it.entity, overrideIsDelete = true) }
        if (tombstones.isEmpty()) return false

        val upsertReq = UpsertReq.builder().databaseName(databaseName).collectionName(collectionName).data(tombstones.map { it.toJsonObject(it.vector.toFloatArray()) }).build()
    ...
    override fun searchByVector(
        ...
        val searchReq = SearchReq.builder()
            .databaseName(databaseName).collectionName(collectionName).annsField("vector").topK(topK)
            .data(listOf(FloatVec(queryVector))).filter(buildFilter(filter))
            .outputFields(listOf("fileNodeId", "vector", "isDelete"))
    ...
    override fun createCollection(collectionName: String) {
        ...
        schema.addField(AddFieldReq.builder().fieldName("fileNodeId").dataType(DataType.VarChar).isPrimaryKey(true).autoID(false).maxLength(128).build())
    ...
    private fun mapToEmbeddingDatum(entity: Map<String, Any?>, overrideIsDelete: Boolean? = null): FileNodeEmbeddingDatum? {
        val vectorValue = entity["vector"] ?: return null
        val vector = convertVector(vectorValue) ?: return null
        val fileNodeId = entity["fileNodeId"]?.toString() ?: return null
        val isDelete = overrideIsDelete ?: (entity["isDelete"] as? Boolean ?: false)
        return FileNodeEmbeddingDatum(vector = vector, fileNodeId = fileNodeId, isDelete = isDelete)
    }
    ...
    private fun FileNodeEmbeddingDatum.toJsonObject(vectorArray: FloatArray): JsonObject {
